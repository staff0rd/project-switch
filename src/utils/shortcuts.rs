...
pub fn collect_shortcuts(
    ...
    for (dir, recursive) in &scan_dirs {
        scan_directory(dir, *recursive, exclude_patterns, &mut seen, &mut entries);
    }
...
fn scan_directory(
    ...
    for entry in read_dir.flatten() {
        ...
        if path.is_dir() && recursive {
            scan_directory(&path, true, exclude_patterns, seen, entries);
            continue;
        }
...
fn matches_any_pattern(name: &str, patterns: &[String]) -> bool {
    ...
    for pattern in patterns {
        ...
        if pat.starts_with('*') && pat.ends_with('*') && pat.len() > 2 {
            ...
            // *suffix — ends with
            if name_lower.ends_with(&pat[1..]) {
                return true;
            }
        } else if pat.ends_with('*') {
            // prefix* — starts with
            if name_lower.starts_with(&pat[..pat.len() - 1]) {
                return true;
            }
        } else {
            // Exact match
