...
- [x] Task: Evaluate GUI framework (iced vs egui/eframe) — build a minimal prototype with text input + scrollable list for both, compare startup time, keystroke latency, binary size, and cross-platform support. Document decision in tech-stack.md.
- [ ] Task: Define the crate structure for the merged binary — decide whether to merge hotkey + launcher into a single crate or use a workspace with shared library crate. Update Cargo.toml(s) accordingly.
